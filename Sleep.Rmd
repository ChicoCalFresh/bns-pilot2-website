...
## Academic Year
**On average, during the academic year, how many hours of sleep do you get in a typical 24-hour period:**
...
```
During the week, students claim to get an average of `r round(mean(bns$q84, na.rm = TRUE), 2)` hours per night, with a median of `r round(median(bns$q84, na.rm = TRUE), 2)` hours. During the weekend, students claim to get an average of `r round(mean(bns$q85, na.rm = TRUE), 2)` hours per night, with a median of `r round(median(bns$q85, na.rm = TRUE), 2)` hours. Students get an average of `r round(mean(bns$q85, na.rm = TRUE), 2)-round(mean(bns$q84, na.rm = TRUE), 2)` more hours of sleep on the weekend versus during the week.
...
```
Students claim that on average they were feeling sleep deprived for `r round(mean(bns$q86, na.rm = TRUE), 1)` days out of the month, with a median of `r round(median(bns$q86, na.rm = TRUE), 1)` days per month. 
...
```{r}
plot_frq(bns$q88) + xlab('Frequency')
```
...
```{r}
plot_frq(bns$q89, ylim=c(0,240)) + xlab('')
```
