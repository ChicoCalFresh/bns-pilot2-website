...
```
The age for participants in the survey range from `r min(bns$age, na.rm = TRUE)` to `r max(bns$age, na.rm = TRUE)` at the time of completion, with the majority of participants being between 20-30 years old. The average age of a participant in this survey (denoted by the dotted line) is `r round(mean(bns$age, na.rm = TRUE),1)` years old.
...
ethnicity_vars %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + xlab("") + ylab("Percent")
```
...
plot_frq(dep.ages.df$Ages, type='histogram', show.mean = TRUE, ylim=c(0,30), xlim=c(0,103), show.mean.val = FALSE, show.sd = FALSE) + 
  geom_boxplot(data=dep.ages.df, aes(x=Ages, y=27), alpha=0.8, width=5, fill="#587db6") +
  geom_jitter(data=dep.ages.df, aes(x=Ages, y=27), position=position_jitter(height=2)) + xlab('Dependent Ages (Years)')
```
From the above graphs, the majority of dependents are under the age of 18. The ages of these dependents ranges from `r min(dep.ages)` to `r max(dep.ages)` with a standard deviation of `r round(sd(dep.ages),2)`. The average age for a dependent (denoted by the dotted line) is `r round(mean(dep.ages),2)` while the median value is `r round(median(dep.ages),2)`. The most common dependent age is `r names(sort(table(dep.ages), decreasing=TRUE)[1])`, which appears `r unname(sort(table(dep.ages), decreasing=TRUE)[1])` times in the data.
...
ggplot(tmp, aes(x=factor(Var1, levels=Var1), y=Freq, fill=FirstGen, label=Percent)) + geom_col(colour = "black") + 
  ...
  xlab('') + theme(legend.position = "none") + ylab('')
```
