...
```
`r fig.cap(page.num, "Student Gender Identity")`
...
```
`r fig.cap(page.num, "Hispanic Origin Identification")`
...
plot_grpfrq(tmp$school, tmp$hispanic, wrap.labels = 25, geom.colors = campus.colors[c(2,1)], legend.title = "Identifies") + xlab("") + theme(legend.position="top")
```
...
`r fig.cap(page.num, "Martial Status Identification")`
...
plot_frq(tmp$firstgen, ylim=c(0,470), coord.flip=TRUE, hjust = -0.1, vjust = 0.3) + xlab('')
```
`r fig.cap(page.num, "First Generation College Students")`
...
plot_grpfrq(bns$school, tmp$firstgen, wrap.labels = 25, ylim=c(0,150), geom.colors = campus.colors[c(2,1)], legend.title = "Status", show.prc = FALSE) + 
  xlab('') + theme(legend.position="top")
```
