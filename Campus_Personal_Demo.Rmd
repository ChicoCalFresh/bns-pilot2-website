...
```{r}
### Within Campus Breakdown
# Table 5.2: First Generation Percentages (within Campus)
# 
# tmp %>% mutate(School=bns$school) %>% group_by(School) %>% count(firstgen) %>% na.omit() %>% 
#   reshape2::dcast(School ~ firstgen) %>% mutate(nfg.percent=percent(.[,2]/apply(.[,2:3], 1, sum)),
#                                                 fg.percent=percent(.[,3]/apply(.[,2:3], 1, sum)),
#                                                 `Not First Gen`=paste0(`Not First Gen`, " (", nfg.percent, ")"),
#                                                 `First Gen`=paste0(`First Gen`, " (", fg.percent, ")")) %>%
#   select(c(1,2,3)) %>% kable() %>% kable_styling(bootstrap_options = "striped") 
# 
# *Above is a breakdown of first gen percentages within campuses (not as a percent of the total).* Looking at the table breakdown, we can see that over 92.0% of students at each campus (that participated in the survey) are considered first generation college students. Less than 8.0% of students are not first generation college students.
```
