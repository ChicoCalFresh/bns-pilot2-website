...
plot_grpfrq(bns$school, bns$q7, wrap.labels = 25, ylim=c(0,104), wrap.legend.labels = 20, geom.colors = likert.pal.3, legend.title = "") + 
...
*Note that below only the values within the table represent "Yes (% of school total)".*
```{r}
...
plot_grpfrq(bns$school, bns$q13, wrap.labels = 25, wrap.legend.labels = 26, geom.colors = RColorBrewer::brewer.pal(8, "Set3"),
...
plot_grpfrq(bns$school, tmp$firstgen, wrap.labels = 25, ylim=c(0,150), geom.colors = campus.colors[c(1,2)], legend.title = "", show.prc = FALSE) + 
  xlab('') + theme(legend.position="top")
```
