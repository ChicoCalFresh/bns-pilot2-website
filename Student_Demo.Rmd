...
```
`r fig.cap(page.num, "Current CSU Enrollment")`
...
```
`r fig.cap(page.num, "Current Academic Level")`
...
```
`r fig.cap(page.num, "Approximate Overall GPA")`
...
plot_grpfrq(bns$school, bns$q17, wrap.labels = 25, geom.colors = RColorBrewer::brewer.pal(5, "Set2"), legend.title = "GPA", show.prc = FALSE) + 
  xlab('') + theme(legend.position="top")
```
...
```{r, fig.width=6.5}
plot_grpfrq(bns$school, bns$q14_1, wrap.labels = 25, ylim=c(0,83), geom.colors = campus.colors[c(2,1)], legend.title = "Identifies", show.prc = FALSE) + 
  xlab('') + theme(legend.position="top")
```
