...
plot_frq(bns$school, ylim=c(0,185), wrap.labels = 16) + xlab('School') + ylab("Frequency")
```
</div>
...
Three schools participated in this pilot of the survey: `r names(sort(table(bns$school), decreasing=TRUE)[1])` (`r unname(sort(table(bns$school), decreasing=TRUE)[1])`), `r names(sort(table(bns$school), decreasing=TRUE)[2])` (`r unname(sort(table(bns$school), decreasing=TRUE)[2])`), and `r names(sort(table(bns$school), decreasing=TRUE)[3])` (`r unname(sort(table(bns$school), decreasing=TRUE)[3])`). The data indicates that students from each campus will be nearly equally represented in the data.
</div>
</div>
...
```{r, fig.width=7}
plot_frq(bns$academic_level, wrap.labels = 8) + xlab('Class Standing') + ylab("Frequency")
```
`r fig.cap(page.num, "Current Academic Level")`

It seems that the majority of the survey participants are either in their junior (`r unname(sort(table(bns$academic_level), decreasing=TRUE)['Junior (60-89 semester units)'])` people) or senior (`r unname(sort(table(bns$academic_level), decreasing=TRUE)['Senior (90+ semester units)'])` people) semester. There are a few sophomores and very few freshman or graduate students.
...
bns %>% select(school, q16) %>% filter(!is.na(q16)) %>% count(q16, school) %>% group_by(school) %>% 
  mutate(freq = paste0("(", percent(n/sum(n), accuracy = 0.1), ")")) %>% 
  ggplot(data = ., aes(x=school, y=n, fill=q16)) + geom_bar(width=0.7, position = position_dodge(width=0.9), stat="identity") + 
  xlab('') + ylab("Frequency") + theme(legend.position="right") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) + 
  scale_fill_manual(labels = function(x) str_wrap(x, width = 20), name="Major", 
                    values=RColorBrewer::brewer.pal(8, "Set2")) + ylim(0,80) +
...
```{r, fig.height=6, fig.width=8}
bns %>% select(school, q17) %>% filter(!is.na(q17)) %>% count(q17, school) %>% group_by(school) %>% 
  mutate(freq = paste0("(", percent(n/sum(n), accuracy = 0.1), ")")) %>% 
  ggplot(data = ., aes(x=school, y=n, fill=q17)) + geom_bar(width=0.7, position = position_dodge(width=0.9), stat="identity") + 
  xlab('') + ylab("Frequency") + theme(legend.position="right") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) + 
  scale_fill_manual(labels = function(x) str_wrap(x, width = 20), name="GPA", 
                    values=RColorBrewer::brewer.pal(8, "Set2")) + ylim(0,77) +
  geom_text(position=position_dodge(width=0.9), aes(y=n + 10, label=paste0(n, " ", freq))) + coord_flip()
```
...
```{r, fig.width=6.5}
bns %>% select(school, q14_1) %>% filter(!is.na(q14_1)) %>% count(q14_1, school) %>% group_by(school) %>% 
  mutate(freq = paste0("(", percent(n/sum(n), accuracy = 0.1), ")")) %>% 
  ggplot(data = ., aes(x=school, y=n, fill=q14_1)) + geom_bar(width=0.7, position = position_dodge(width=0.9), stat="identity") + 
  xlab('') + ylab("Frequency") + theme(legend.position="top") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) + 
  scale_fill_manual(labels = function(x) str_wrap(x, width = 20), name="Transfer Student", 
