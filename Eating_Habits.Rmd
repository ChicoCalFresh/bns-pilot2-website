...
```{r, fig.height=5.5, fig.width=7.7}
schools <- c("California State University, Chico", "California State University, Fresno",
             "California State University, Northridge")

for (i in 1:length(schools)) {
  tmp <- bns %>% filter(school == schools[i])
  questions <- tmp %>% select(starts_with("q76_"))
  names(questions) <- c("I do not regularly have access to a kitchen where I can prepare meals.",
                        "I have a secure place to store fresh food and pantry items.",
                        "I do not have enough room to store food items.",
                        "Thinking about food is stressful/causes anxiety.",
                        ...
                        "No one cooks at home.",
                        "I do not think about my eating/food habits.")
  plt <- plot_likert(questions, grid.range = c(1.2, 1.2), wrap.legend.labels = 20,
                     expand.grid = FALSE, values = "sum.outside", catcount = 5,
                     geom.colors = likert.pal.5, show.prc.sign = TRUE, reverse.scale = TRUE,
                     title = schools[i]) +
...
rm(schools, tmp, plt)
```
`r fig.cap(page.num, "Eating/Cooking Habits by Campus")`
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of Daily Fruits & Vegetables. </summary>
```{r, fig.width=6.2, fig.height=4}
plot_grpfrq(bns$school, bns$fruit_veg_serv_daily, geom.colors = RColorBrewer::brewer.pal(4, "Set2"),
            wrap.labels = 21, legend.title = "Servings", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "Daily Fruits & Vegetables by Campus")`
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of Daily Sugar/Energy Drinks. </summary>
```{r, fig.width=7, fig.height=4.7}
plot_grpfrq(bns$school, bns$sugar_drinks_ounces_daily,
            wrap.labels = 28, legend.title = "Ounces", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "Daily Sugar/Energy Drinks by Campus")`

```{r}
bns %>% group_by(school) %>% 
  summarise(tmp = percent(mean(sugar_drinks_ounces_daily == "0", na.rm = TRUE), accuracy = .1)) %>%
  rename("School" = school, "Don't Consume Sugar/Energy Drinks" = tmp) %>%
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of weekly eating out. </summary>
```{r, fig.width=7, fig.height=4.7}
plot_grpfrq(bns$school, bns$eating_out_weekly,
            wrap.labels = 28, legend.title = "Frequency", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "Weekly Eating Out by Campus")`

```{r}
bns %>% group_by(school) %>% 
  summarise(tmp = percent(mean(eating_out_weekly == "1-2", na.rm = TRUE), accuracy = .1)) %>%
  rename("School" = school, "Eating Out 1-2 Times Weekly" = tmp) %>%
  kable() %>% kable_styling(bootstrap_options = "striped")
```
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of weekly alcohol consumption. </summary>
```{r, fig.width=7, fig.height=4.7}
plot_grpfrq(bns$school, bns$alc_consumption_weekly,
            wrap.labels = 28, legend.title = "Frequency", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "Weekly Drinking by Campus")`

```{r}
bns %>% group_by(school) %>% 
  summarise(tmp = percent(mean(alc_consumption_weekly == "0", na.rm = TRUE), accuracy = .1)) %>%
  rename("School" = school, "Do Not Consume Alcohol" = tmp) %>%
  kable() %>% kable_styling(bootstrap_options = "striped")
```
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of food comfort. </summary>
```{r, fig.width=7.5}
schools <- c("California State University, Chico", "California State University, Fresno",
             "California State University, Northridge")
...
for (i in 1:length(schools)) {
  tmp <- bns %>% filter(school == schools[i])
  questions <- tmp %>% select(starts_with("q82_"))
  names(questions) <- c("How confident are you that you can use the same healthy ingredient in more than one meal?",
                      "How confident are you that you can use basic cooking skills, like cutting fruits and vegetables, measuring out ingredients, or following a recipe?",
                      "How confident are you that you can buy healthy foods for your family on a budget?",
                      "How confident are you that you can cook healthy foods for your family on a budget?")
  plt <- plot_likert(questions, grid.range = c(1, 1), wrap.legend.labels = 20, expand.grid = FALSE,
                     values = "sum.outside", catcount = 5, geom.colors = likert.pal.5,
                     show.prc.sign = TRUE, reverse.scale = TRUE, title = schools[i]) +
    theme_bw() + theme(legend.position = "top")
  print(plt)
}
rm(schools, tmp, plt)
```
`r fig.cap(page.num, "Food Comfort by Campus")`
