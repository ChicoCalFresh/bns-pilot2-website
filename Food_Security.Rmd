...
```{r, fig.width=7}
binary_table_2 <- function(var, value, rnames) {
  tmp <- as.data.frame(t(bns[questions]))
  tmp2 <- data.frame(Freq=apply(tmp, 1, function(x, value) sum(x == value, na.rm=TRUE), "Yes"))
  ...
  rownames(tmp2) <- paste0(rnames, " (n = ", n.s, ")?")
  colnames(tmp2) <- "Yes (%)"
...
binary_table_2(questions, "Yes", rnames)
```
