...
```
Figure 3.1: Weekly Paid Hour Ranges
...
```
Figure 3.2: Weekly Paid Hours Distribution
...
```
Figure 3.3: COVID Impact on Paid Hours
...
```
Figure 3.4: Weekly Unpaid Hour Ranges
...
plot_frq(tmp$q21, type='histogram', geom.size = 1, xlim = c(1,20), ylim=c(0,18), show.prc = FALSE, show.n = FALSE, 
  ...
  scale_x_continuous(breaks=c(seq(1,40,5), 41), labels=xlbls) + xlab('Weekly Hours')
```
Figure 3.5: Weekly Unpaid Hours Distribution
...
```
Figure 3.6: Weekly Caregiving Hour Ranges
...
plot_frq(tmp$q22, type='histogram', geom.size = 1, xlim = c(1,20), show.prc = FALSE, show.n = FALSE, 
  ...
  scale_x_continuous(breaks=c(seq(1,40,5), 41), labels=xlbls) + xlab('Weekly Hours')
```
Figure 3.7: Weekly Caregiving Hours Distribution
...
tmp2$TotalHours <- ifelse(is.na(tmp2[, 'q18']) & is.na(tmp2[,'NonAcademic']), NA,  rowSums(tmp2[, c('q18','NonAcademic')], na.rm = TRUE))
```
...
```
Figure 3.8: Academic Hours Distribution
...
```
Figure 3.9: Non-Academic Hours Distribution
...
```
Figure 3.10: Total Workload Hours Distribution
...
```
Figure 3.11: CalFresh Workload Qualification Breakdown
