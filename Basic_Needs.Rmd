...
```{r, fig.width=6.5}
tmp <- bns %>% group_by(fed_through_EBT) %>% summarise(avg = mean(q49_1, na.rm=TRUE))

ggplot(tmp, aes(x=fed_through_EBT, y=avg)) + geom_bar(stat = "identity", fill="#336699") +
  geom_text(aes(y=avg + 16, label=paste0("$", round(avg, 2)))) + 
  xlab('Household Size') + ylab('Monthly Assistance ($)') + 
  scale_x_discrete(limits=seq(1,8))
```
`r fig.cap(page.num, "Monthly Assistance per Household")`
