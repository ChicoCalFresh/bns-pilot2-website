...
plot_frq(bns$q23, ylim=c(0,315)) + xlab('')
```
</div>
...
<details>
<summary> Click here for campus breakdown of COVID housing impact. </summary>
```{r, fig.width=5, fig.height=3.8}
plot_grpfrq(bns$school, bns$q23, geom.colors = campus.colors[c(2,1)], wrap.labels = 21,
            legend.title = "Housing Impacted", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "COVID Housing Impact by Campus")`

From the figure above, we can see Chico has slightly under twice as many students whose housing was not impacted by COVID. Fresno has slightly over three times as many students whose housing was not impacted by COVID. And Northridge has around 1.5 times as many students whose housing was not impacted by COVID.
...
<div class = "col-md-6">
```{r, fig.height=3, fig.width=4}
plot_frq(bns$q24, ylim=c(0,285)) + xlab('')
```
</div>
...
### `r emoji.1` `r emoji.2` Campus Breakdown
<details>
<summary> Click here for campus breakdown of living near enrolled campus. </summary>
```{r, fig.width=5, fig.height=3.8}
plot_grpfrq(bns$school, bns$q24, geom.colors = campus.colors[c(2,1)], wrap.labels = 21,
            legend.title = "Live Near College", show.prc = FALSE) + xlab('') + 
  theme(legend.position="top")
```
`r fig.cap(page.num, "Living Near College by Campus")`

From the figure above, we can see that all campuses have the majority of there students living near their enrolled campus. Fresno and Northridge seem to have slightly less students living near campus compared to Chico.
