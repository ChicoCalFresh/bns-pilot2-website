...
huxtable(tmp, add_rownames = "") %>% set_background_color(2:11, 2:9, "red") %>%
  set_background_color(2:3, 2, "green") %>% set_background_color(2:4, 3, "green") %>% 
  set_background_color(2:5, 4, "green") %>% set_background_color(2:6, 5, "green") %>% 
  set_background_color(2:7, 6, "green") %>% set_background_color(2:8, 7, "green") %>% 
  set_background_color(2:9, 8, "green") %>% set_background_color(2:10, 9, "green") %>% 
  set_all_borders(brdr(3, "solid", "white")) %>% map_align(by_cols("center")) %>%
  set_width(0.75) %>% set_font_size(12)

rm(tmp)
```
*In the above graph, cells highlighted in green indicate a person is eligible for CalFresh based on their income and cells highlighted in red indicate a person is not eligible. This information is pulled from the CalFresh requirements section A2 (see below for link).*
...
- Falls under the household size/income from section A2 on the [CalFresh Basic Eligibility](https://www.csuchico.edu/calfresh/basics/eligibility.shtml).
